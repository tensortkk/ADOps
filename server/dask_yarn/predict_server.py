...
if __name__ == "__main__":
    ...
    parser.add_argument("-db", "--database", help="specify the database", default="adops")
    parser.add_argument("-dp", "--deployment", help="specify the deployment", default="d1")
    parser.add_argument("-m", "--model_path", help="specify the model path", default="/work/adops/saved_models/xgbmodel.json")
    parser.add_argument("-t", "--model_type", help="specify the model type", default="xgboost")
    parser.add_argument("-s", "--schema", help="specify the schema")
    parser.add_argument("-p", "--port", help="specify the model port", default=8881)
    parser.add_argument("-n", "--name", help="the name of alias")
                            
    args = parser.parse_args()
    url = f"http://{args.endpoint}/dbs/{args.database}/deployments/{args.deployment}"
    global model
    table_schema = args.schema.split(",")
    for i in range(len(table_schema)):
        table_schema[i] = table_schema[i].split(":")
    if args.model_type == "xgboost":
        from xgboost.sklearn import XGBClassifier
        model = XGBClassifier()
        model.load_model(fname=args.model_path)
